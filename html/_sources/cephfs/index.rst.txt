...
现在，又加入了两个其它的存储接口，形成了一个现代的、大一统的存储系统：
...
如果后端的部署技术支持的话（见 `Orchestrator 部署表`_ ），
Ceph `Orchestrator`_ 会自动给你的文件系统创建和配置 MDS ，
否则，请 `根据需要手动部署 MDS`_ 。你还可以\ `创建其他 CephFS 卷`_\ 。

最后，要想在你的客户端节点上挂载 CephFS ，参见 `挂载 CephFS ：先决条件`_ 。
另外，还有一个命令行的 shell 工具，可以用 `cephfs-shell`_ 交互地访问或者编写脚本。

.. _Orchestrator: ../mgr/orchestrator
.. _根据需要手动部署 MDS: add-remove-mds
.. _创建其他 CephFS 卷: fs-volumes
.. _Orchestrator 部署表: ../mgr/orchestrator/#current-implementation-status
