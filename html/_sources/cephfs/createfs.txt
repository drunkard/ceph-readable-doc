...
.. tip::
    ...
    ``ceph fs new`` 命令是从 Ceph 0.84 起引入的，在此之前，无\
    需手动创建文件系统，名为 ``data`` 和 ``metadata`` 的存储池\
    默认即存在。

    Ceph 命令行现在有了创建和删除文件系统的命令，但是当前一套\
    集群只能有一个文件系统存在。

一个 Ceph 文件系统需要至少两个 RADOS 存储池，一个用于数据、一\
个用于元数据。配置这些存储池时需考虑：

- 为元数据存储池设置较高的副本水平，因为此存储池丢失任何数据都\
  会导致整个文件系统失效。
- 为元数据存储池分配低延时存储器（像 SSD ），因为它会直接影响\
  到客户端的操作延时。
