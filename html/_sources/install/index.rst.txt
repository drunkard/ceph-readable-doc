...
:ref:`Cephadm <cephadm_deploying_new_cluster>` 是专门用于安装和管理 Ceph 集群的工具。

* cephadm 只支持 Octopus 和更新的版本。
* cephadm 已经和新的编排 API 完整对接了、
  并且完整支持 CLI 和仪表盘功能，以此来管理集群部署。
* cephadm 需要容器功能的支持（ Podman 或 Docker 格式的）、还有 Python 3 。
* cephadm 依赖 systemd 。
...
* Rook 是在 Kubernetes 上运行 Ceph 、
  或者把 Kubernetes 集群连接到一个已有（外部的） Ceph 集群的首选方式。
...
* ceph-ansible 还没有和新的编排器 API 对接好，
  它是在 Nautilus 和 Octopus 才引进的，
  这就意味着用 ceph-ansible 方式部署的话，
  从 Nautilus 和 Octopus 起引进的管理功能和仪表盘对接还不能用。


`ceph-deploy <https://docs.ceph.com/projects/ceph-deploy/en/latest/>`_
是个工具，用于快速部署集群。
...
`OpenNebula HCI 集群 <https://docs.opennebula.io/stable/provision_clusters/hci_clusters/overview.html>`_ 可在各种云平台上部署 Ceph 。
