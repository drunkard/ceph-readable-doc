...
**ceph-post-file** 可把文件或目录上传到 ceph.com
让 Ceph 开发者稍后分析。

每次调用都会把文件或目录上传到一个带惟一标签的独立目录。
此标签可共享给开发者或在缺陷报告（ http://tracker.ceph.com/ ）里引用。
一旦上传完成，目录就被标记为不可读且不可写，
以防其它用户访问或修改。
