...
:command:`device map` [-t | --device-type *device-type*] [--cookie *device-cookie*] [--show-cookie] [--snap-id *snap-id*] [--read-only] [--exclusive] [-o | --options *device-options*] *image-spec* | *snap-spec*
  ...
  或其它支持的设备
  （ Linux 上的 *nbd* / *ubbd* 或 FreeBSD 上的 *ggate* ）。
...
:command:`merge-diff` *first-diff-path* *second-diff-path* *merged-diff-path*
  ...
  注意，当前此命令只支持 stripe-count == 1 这样的源增量差异。
...
.. option:: stripe-count
   ...
   我们把 [*stripe_unit*] 个字节写够 [*stripe-count*] 个对象\
   后，再转回到第一个对象写另一轮条带，直到达到对象的最大尺\
   寸。此时，我们继续写下一轮 [*stripe-count*] 个对象。

默认情况下， [*stripe_unit*] 和对象尺寸相同、且 [*stripe-count*] 为 1 ；
另外指定 [*stripe_unit*] 和/或 [*stripe-count*] 通常出现在使用 fancy 条带化时、
