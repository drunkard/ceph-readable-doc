...
一致，但是你可以增加、删除、或替换集群中的监视器，
详情见\ :ref:`adding-and-removing-monitors` 。
...
:term:`集群运行图`\ 的存在使得 :term:`Ceph 客户端`\ 能够确定\
所有 Ceph 监视器、 Ceph OSD 守护进程和 Ceph 元数据服务器的位置。
客户端只要连到其中一个 Ceph 监视器并获取一份当前的集群运行图即可。
Ceph 客户端必须先连到一个监视器，然后才能读写 OSD 或元数据服务器。
用当前的集群运行图副本和 CRUSH 算法，
客户端能计算出集群内任意对象的位置，
这样， Ceph 客户端就有能力直接与 Ceph OSD 守护进程通讯。
客户端与 Ceph OSD 守护进程直接通讯是重大改进，
传统存储架构下客户端必须和一个中央组件通讯。
更多信息见\ :ref:`arch_scalability_and_high_availability` 。

监视器的主要角色是维护集群运行图的主副本，
它也提供认证和日志记录服务。
Ceph 监视器们把监视器服务的所有更改都写入一个单独的 Paxos 例程，
然后 Paxos 以键/值方式存储所有变更以实现高度一致性。
同步期间， Ceph 监视器能查询集群运行图的近期版本，\
它们通过操作键/值存储快照和迭代器
（用 RocksDB ）来进行存储级同步。
...
守护进程的主机混用。因为 RocksDB 靠 ``mmap()`` 写数据， Ceph
...
新监视器加入集群时有必要进行同步。
在运行中，监视器会不定时收到集群运行图的更新，
这就意味着 leader 和 provider 角色可能在监视器间变幻。
...
时钟漂移即使尚未造成损坏也能被 NTP 感知，
Ceph 的时钟漂移或时钟偏差警告即使在 NTP 同步水平合理时也会被触发。
提高时钟漂移值有时候尚可容忍，
然而很多因素（像载荷、网络延时、覆盖默认超时值和\
`监视器存储同步`_\ 选项）都能\
在不降低 Paxos 保证级别的情况下影响可接受的时钟漂移水平。
