...
监视器们维护着集群运行图的“主副本”，就是说客户端连到一个监视器\
并获取当前运行图就能确定所有监视器、 OSD 和元数据服务器的位置。
Ceph 客户端读写 OSD 或元数据服务器前，必须先连到一个监视器，\
靠当前集群运行图的副本和 CRUSH 算法，客户端能计算出任何对象的\
位置，故此客户端有能力直接连到 OSD ，这对 Ceph 的高伸缩性、\
高性能来说非常重要。更多信息见\ `伸缩性和高可用性`_\ 。
