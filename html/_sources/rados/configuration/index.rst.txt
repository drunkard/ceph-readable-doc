...
Ceph 作为集群时可以包含数千个对象存储设备（ OSD ）。最简系统\
至少需要二个 OSD 做数据复制。要配置 OSD 集群，你得把配置写入\
配置文件。 Ceph 对很多选项提供了默认值，你可以在配置文件里\
覆盖它们；另外，你可以使用命令行工具修改运行时配置。
...
各进程、守护进程或工具都会读取配置文件。一个进程可能需要不止\
一个守护进程例程的信息（\ *即*\ 多个上下文）；一个守护进程或\
工具只有关于单个守护进程例程的信息（单上下文）。
