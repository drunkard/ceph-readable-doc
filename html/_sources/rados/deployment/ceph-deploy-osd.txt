...
创建集群、安装 Ceph 软件包、收集密钥完成后你就可以准备 OSD 并\
把它们部署到 OSD 节点了。如果你想确认某磁盘或擦净它，参见\
`列举磁盘`_\ 和\ `擦净磁盘`_\ 。 ::
...
``prepare`` 命令只准备 OSD 。在大多数操作系统中，硬盘分区创建\
后，不用 ``activate`` 命令也会自动执行 ``activate`` 阶段（通过
Ceph 的 ``udev`` 规则）。详情见\ `激活 OSD`_\ 。

前例假定一个硬盘只会用于一个 OSD 守护进程，以及一个到 SSD 日志\
分区的路径。我们建议把日志存储于另外的驱动器以最优化性能；你也\
可以指定一单独的驱动器用于日志（也许比较昂贵）、或者把日志放到
OSD 数据盘（不建议，因为它有损性能）。前例中我们把日志存储于分\
好区的固态硬盘。

你可以用 ``--fs-type`` 或 ``--bluestore`` 选项来指定 OSD 所用\
的文件系统。（具体请看 ``ceph-deploy osd prepare --help`` ）
...
``activate`` 命令会让 OSD 进入 ``up`` 且 ``in`` 状态，此命令所\
用路径和 ``prepare`` 相同。
