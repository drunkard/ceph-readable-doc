...
Ceph 通过 RADOS 集群动态地存储、复制、并重新均衡数据对象。
它可以让很多不同用户把各种对象存储到无数 OSD 之上的不同的存储池里，
以实现各自的目标，所以 Ceph 的运营需要些数据归置计划。
Ceph 的数据归置计划概念主要有：

- **存储池（ Pool ）：** Ceph 在存储池内存储数据，
  它是对象存储的逻辑组；存储池管理着归置组数量、
  副本数量、和存储池的 CRUSH 规则。
  要往存储池里存数据，用户必须通过认证、且在这个存储池的权限合适。
  存储池可做快照。详情参见\ :ref:`rados_pools` 。
...
- **CRUSH 图（ CRUSH Map ）：** CRUSH 是重要组件，它使 Ceph 能\
  ...
  本应该在哪里、怎样跨故障域存储，以提升数据安全。
  详情见 :ref:`rados-crush-map` 。
