...
在主机的物理数据中心、房间、行和机架，据此你可以请求在线支持\
或替换硬件。
 ...
 * 如果分级结构的支部只有少量设备，一些 PG 的副本数小于期望\
   值，这通常出现在一些子结构里， host 节点下少数 OSD 嵌套到\
   了其他 OSD 里。

 * 大型集群里，小部分 PG 映射到的 OSD 数目小于期望值，有多层\
   结构（如：机架行、机架、主机、 OSD ）时这种情况更普遍。

 * 当一些 OSD 标记为 out 时，数据倾向于重分布到附近 OSD 而非\
   整个分级结构。
 ...
 * 对于已经在运行的集群，里面已经有了大量数据，从 0 改为 1
