...
高级集群操作主要包括用 ``ceph`` 服务管理脚本启动、停止、重启\
集群，和集群健康状态检查、监控和操作集群。
...
低级集群运维包括启动、停止、重启集群内的某个具体守护进程；更改\
某守护进程或子系统配置；增加或拆除守护进程。低级运维还经常遇到\
扩展、缩减 Ceph 集群，以及更换老旧、或损坏的硬件。
...
Ceph 仍在积极开发中，所以你可能碰到一些问题，需要评估 Ceph 配\
置文件、并修改日志和调试选项来纠正它。
