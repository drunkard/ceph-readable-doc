...
从 Luminous 版（ 12.2.0 ）之后，我们有了新的稳定版发布周期，\
是每 9 个月。各系列的稳定版都会取个名字（如 Mimic ）、和一个\
主版本号（如 Mimic 的是 13 ，因为 M 是字母表的第十三个）。

版本名是以头足类动物命名的（一般是通用名，因为拉丁名不好记、\
也不好拼读）。
...
* 完整的集成测试
* 从上一个稳定版升级的测试
* 我们尽最大努力，以确保从前一个开发版可以成功地\ *离线*\ 升\
  级（就是说你可以停掉所有守护进程、升级、然后重启）；开发版\
...
在上述的时间表中，一个稳定版系列的寿命从首次发布的那个月算起，\
差不多是 18 个月（即两个 9 月发布周期）。例如， Luminous
（12.2.z）在 Nautilus （ 14.2.0 ）发布后不久就会走到生命的尽头\
（ EOL ）。一个版本的寿命可能差别巨大，因为它取决于稳定版的发\
布速度。
...
* 从 Jewel 或 Kraken 开始升级的话，必须先升级到 Luminous 才能\
  ...
  Mimic ）。
* Jewel 将会一直维护，直到 2018 年春天 Mimic 发布为止；
* Kraken 就不再维护了。
