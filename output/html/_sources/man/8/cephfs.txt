...
设置布局数据时，指定的对象尺寸必须被指定的条带单位平均分割，未指定的参数都按系统默认\
值。

显式地设置一文件和目录布局的含义不同。设置一文件的布局指示如何放置这一个文件，而且必\
须在存入数据\ *前*\ 就设置好。删减文件也不允许你更改其布局。

设置一目录的布局设置了“默认布局”，它将用于设置此目录内后续新建文件（或子目录）的\
文件布局。先前已存在文件的布局不会变。

也许你注意到了布局信息允许你指定优先使用的 OSD 。此功能已废弃，较新版本的 Ceph 服务\
器会忽略它，别用。
