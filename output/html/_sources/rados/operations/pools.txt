...
- **自恢复力：** 你可以设置在不丢数据的前提下允许多少 OSD 失效，对多副本存储池来\
  说，此值是一对象应达到的副本数。典型配置存储一个对象和它的一个副本（即 \
  ``size = 2`` ），但你可以更改副本数；对\ `带纠删码的存储池`_\ 来说，此值是编码块\
  数（即\ **纠删码配置**\ 里的 ``m=2`` ）。
...
:Description: 存储池名称，必须唯一。
:Type: String
:Required: 必需。没指定的话读取默认值、或 Ceph 配置文件里的值。


``{pg-num}``

:Description: 存储池拥有的归置组总数。关于如何计算合适的数值，请参见\ `归置组`_\ 。\
              默认值 ``8`` 对大多数系统都不合适。
...
:Description: 用于归置的归置组总数。此值\ **应该等于归置组总数**\ ，归置组分割的情\
              况下除外。

:Type: Integer
:Required: 没指定的话读取默认值、或 Ceph 配置文件里的值。
:Default: 8


``{replicated|erasure}``

:Description: 存储池类型，可以是\ **副本**\ （保存多份对象副本，以便从丢失的 OSD \
...
:Description: 此存储池所用的 crush 规则集名字。若指定的规则集不存在，\ **副本**\ \
              存储池的创建就会失败，原因为 -ENOENT ；但是\ **纠删码**\ 存储池会以\
              此名字新建一个删除码规则集。

:Type: String
:Required: No.
:Default: **纠删码存储池**\ 为 "erasure-code" ；\ **副本**\ 存储池为 \
          **osd_pool_default_crush_replicated_ruleset** 。


``[erasure-code-profile=profile]``

:Description: 仅用于\ **纠删**\ 存储池。指定\ `纠删码配置`_\ 框架，此配置必须已由 \
...
:Description: 设置存储池中的对象副本数，详情参见\ `设置对象副本数`_\ 。仅适用于副本存储池。
:Type: Integer


``min_size``

:Description: 设置 I/O 需要的最小副本数，详情参见\ `设置对象副本数`_\ 。仅适用于副本存储池。
:Type: Integer
:Version: ``0.54`` 及以上。
...
:Description: 允许客户端重放确认而未提交请求的秒数。
:Type: Integer


``pgp_num``

:Description: 计算数据归置时使用的有效归置组数量。
:Type: Integer
:Valid Range: 等于或小于 ``pg_num`` 。


``crush_ruleset``

:Description: 集群内映射对象归置时使用的规则集。
:Type: Integer


``hashpspool``

:Description: 给指定存储池设置/取消 HASHPSPOOL 标志。
:Type: Integer
:Valid Range: 1 开启， 0 取消
:Version: ``0.48`` 及以上。


``hit_set_type``

:Description: 启用缓存存储池的命中集跟踪，详情见 `Bloom 过滤器`_\ 。
:Type: String
:Valid Settings: ``bloom``, ``explicit_hash``, ``explicit_object``
:Default: ``bloom`` ，其它是用于测试的。


``hit_set_count``

:Description: 为缓存存储池保留的命中集数量。此值越高， ``ceph-osd`` 守护进程消耗的内存越多。
:Type: Integer
...
:Description: 为缓存存储池保留的命中集有效期。此值越高， ``ceph-osd`` 消耗的内存越多。
:Type: Integer
:Example: ``3600`` 1hr
...
:Description: ``bloom`` 命中集类型的假阳性概率。详情见 `Bloom 过滤器`_\ 。
:Type: Double
:Valid Range: 0.0 - 1.0
:Default: ``0.05``
...
:Description: 缓存存储池包含的脏对象达到多少比例时就把它们回写到后端的存储池。
:Type: Double
:Default: ``.4``
...
:Description: 缓存存储池包含的干净对象达到多少比例时，缓存代理就把它们赶出缓存存储池。
:Type: Double
:Default: ``.8``
...
:Description: 达到 ``max_bytes`` 阀值时 Ceph 就回写或赶出对象。
:Type: Integer
...
:Description: 达到 ``max_objects`` 阀值时 Ceph 就回写或赶出对象。
:Type: Integer
...
:Description: 达到此时间（单位为秒）时，缓存代理就把某些对象从缓存存储池刷回到存储池。
:Type: Integer
:Example: ``600`` 10min
...
:Description: 达到此时间（单位为秒）时，缓存代理就把某些对象从缓存存储池赶出。
:Type: Integer
:Example: ``1800`` 30min
...
:Description: 存储池的归置组数量。
:Type: Integer


``pgp_num``

:Description: 计算数据归置时使用的归置组有效数量。
:Type: Integer
:Valid Range: 等于或小于 ``pg_num`` 。
