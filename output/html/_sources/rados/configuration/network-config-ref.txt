...
:term:`元数据服务器`\ 会监听公共网 6800 以上的第一个可用端口，确保打开元数据服务器\
上 6800 以上的端口。按下例增加规则时，要把 ``{iface}`` 替换为公共网接口（如 \
...
OSD 默认\ `绑定`_ 6800 以上的第一个可用端口，要确保为各 OSD 至少打开了 3 个 6800 \
以上的端口。一主机上的各个 OSD 最多会用到 3 个端口：
...
各节点的端口不尽相同，所以你不需要开放多于 Ceph 所需的端口。但是最好多开放一点，因\
为有时候某个守护进程失败了，但是重启前并未全部释放，以致于重启后的进程监听了新端口。
...
绑定选项用于设置 OSD 和 MDS 默认使用的端口范围，默认范围是 ``6800:7100`` 。确保\ \
`防火墙`_\ 开放了对应端口范围。
